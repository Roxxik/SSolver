...
scoring :: Board -> Int
scoring = scoreTyped scoringMethod

scoreTyped :: Int -> Board -> Int
scoreTyped p xs	| p == 0 = scoreFlow xs * countZeros xs
				| p == 1 = scoreFlow xs + countZeros xs
				| p == 2 = scoreFlow xs * if countZeros xs <= 3 then 1 else 3
--				| p == 3 = scoreFlow xs * maxCornered xs
--				| p == 4 = scoreFlow xs * maxCornered xs * countZeros xs
...
scoreFlowHor f xs = sum $ map (scoreFlowRow scoreFlowMethod f) xs

scoreFlowRow :: Int -> (Stone -> Stone -> Bool) -> [Stone] -> Int
scoreFlowRow _ _ []  = 0
scoreFlowRow _ _ [x] = 0
scoreFlowRow p f (x:xs)	| f x (head xs) && p == 0 = 1 + scoreFlowRow p f xs
						| f x (head xs) && p == 1 = x + scoreFlowRow p f xs
						| f x (head xs) && p == 2 = (head xs) + scoreFlowRow p f xs
						| f x (head xs) && p == 3 = x + (head xs) + scoreFlowRow p f xs
						| f x (head xs) && p == 4 = x * (head xs) + scoreFlowRow p f xs
					  	| otherwise	  = scoreFlowRow p f xs
