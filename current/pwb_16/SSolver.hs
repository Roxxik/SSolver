...
chooseMoove (u:l:d:r:xs)| d == y = Down
						| l == y = Left
						| r == y = Right
						| u == y = Up
...
rekurScoring d xs n = avgScore((scoring ys) : (scoreBoardRekur ys (n-1)))
							where ys = move xs d
...
scoring :: Board -> Score
scoring xs = (scoreFlow xs + 1) * (countZeros xs + 1) * (length (possibleMoves xs) + 1)

avgScore :: [Score] -> Score
avgScore xs = round(fromIntegral(sum xs) / fromIntegral(length(filter (/= 0) xs)))
...
scoreFlowBoth f xs = avgScore[scoreFlowHor f xs, ((scoreFlowHor f).transpose) xs]
