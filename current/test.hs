...
makeMove xs = chooseMove $ scoreBoardRekur xs rekurDepth

chooseMove :: [Int] -> Direction
chooseMove (u:l:d:r:xs)| u == y = Up
						| l == y = Left
						| d == y = Down
						| r == y = Right
...
scoring :: Board -> Int
scoring xs = scoreFlow xs * countZeros xs + round (1000 / fromIntegral(scoreSmoothness xs))
...
scoreSmoothness xs = if y == 0 then 1 else y
	where y = avgInt(map (avgInt.(map scoreNeighbour)) (getNeighbours xs))
